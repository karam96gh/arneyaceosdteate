// src/middleware/auth.js - ENHANCED VERSION مع إصلاح مشكلة multer
...
// ✅ التحقق من الـ token - ENHANCED VERSION
const requireAuth = async (req, res, next) => {
    ...
    try {
        // ✅ إذا كان req.user موجود بالفعل، لا نحتاج إعادة التحقق (مفيد مع multer)
        if (req.user && req.user.id && req.user.role) {
            console.log('✅ User already authenticated, skipping re-verification:', {
                id: req.user.id,
                role: req.user.role,
...
const requireRole = (roles) => {
    return (req, res, next) => {
        ...
        if (!req.user) {
            console.log('❌ No user in request - calling auth middleware');
            // ✅ إذا لم يكن المستخدم موجود، استدعاء auth middleware
...
const requirePropertyOwnership = async (req, res, next) => {
    try {
        ...
        if (!req.user) {
            ...
            return requireAuth(req, res, (err) => {
                if (err) return next(err);
                return requirePropertyOwnership(req, res, next);
            });
        }
...
const requireBuildingOwnership = async (req, res, next) => {
    try {
        ...
        if (!req.user) {
            return requireAuth(req, res, (err) => {
                if (err) return next(err);
                return requireBuildingOwnership(req, res, next);
            });
        }
...
const requireBuildingItemOwnership = async (req, res, next) => {
    try {
        ...
        if (!req.user) {
            return requireAuth(req, res, (err) => {
                if (err) return next(err);
                return requireBuildingItemOwnership(req, res, next);
            });
        }
...
module.exports = { 
    ...
    testAuth, // للاختبار
    extractToken // تصدير دالة استخراج التوكن للاستخدام في middlewares أخرى
};